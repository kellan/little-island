...
import { advance, alpha, createAccumulator } from './timestep';
import { createStressWorld, hashStress, stepStress } from './stress-simulation';
import { createWorld, orderChop, step } from './simulation';
...
describe('fixed timestep', () => {
  ...
  it('lands the villager in the same place at 60 and 30 frames per second', () => {
    const fast = createWorld(), slow = createWorld();
    orderChop(fast, 3); orderChop(slow, 3);
    run(1200, 1 / 60, dt => step(fast, dt));
    run(600, 1 / 30, dt => step(slow, dt));
    expect(slow.logs).toBe(fast.logs);
    expect(slow.villager.x).toBe(fast.villager.x);
    expect(slow.villager.z).toBe(fast.villager.z);
    expect(slow.time).toBe(fast.time);
  });
