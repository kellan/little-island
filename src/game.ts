...
try{const saved=localStorage.getItem('littlewood-v1');if(saved)world=deserialize(saved)??world;}catch{/* Storage can be unavailable in private browsing. */}
let paused=false,speed=1,sound=false,audio:AudioContext|undefined,selected:number|null=null;
...
<div id="tooltip" class="tree-tooltip" hidden>${svg('tree')}<span>Select to gather</span></div>
...
const view=new IslandScene($<HTMLCanvasElement>('#world'),world);
function toast(message:string){$('#toast').textContent=message;$('#toast').classList.add('visible');window.clearTimeout(toastTimer);toastTimer=window.setTimeout(()=>$('#toast').classList.remove('visible'),3200);}let toastTimer=0;
...
function save(){try{localStorage.setItem('littlewood-v1',serialize(world));}catch{/* The current session still works without storage. */}}
...
canvas.addEventListener('pointermove',event=>{if(event.buttons)return;const id=view.pick(event.clientX,event.clientY);selected=id;view.selected=id;canvas.style.cursor=id===null?'grab':'pointer';const tip=$('#tooltip');tip.hidden=id===null;if(id!==null){tip.style.left=`${Math.min(innerWidth-170,event.clientX+18)}px`;tip.style.top=`${event.clientY-44}px`;}});
...
function frame(now:number){const elapsed=(now-previous)/1000;previous=now;if(!paused)advance(clock,elapsed*speed,dt=>step(world,dt));view.update(now/1000);
  ...
  if(world.logs>previousLogs){note(659,.25);toast(world.logs===1?'Your first log. Every little world starts somewhere.':'+1 timber · safely home');save();}previousLogs=world.logs;
  const chopBeat=Math.floor(world.time*1.43);if(v.phase==='chopping'&&!paused&&chopBeat!==lastChop){note(150,.05);lastChop=chopBeat;}
